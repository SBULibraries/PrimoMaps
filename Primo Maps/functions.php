...
function go($image){
    ...
    if($debug){
        ...
        if (is_array($image)) {
            header("Location: https://repo.api.library.stonybrook.edu/MATT/ui.php?image={$image[2]}&xp={$image[0]}&yp={$image[1]}&call=$call_number&si={$image[3]}&title=Map");
        } else {
            header("Location: https://repo.api.library.stonybrook.edu/MATT/ui.php?image=$image&call=$call_number&title=Map");
        }
    }
...
function normalize_lc($lc, &$oversize = false){
    ...
    #Stolen from UND who stole it from http://robotlibrarian.billdueber.com/2008/11/normalizing-loc-call-numbers-for-sorting/
    $pattern = "/^
              \s*
              ([A-Z]{1,3})  # alpha
              \s*
